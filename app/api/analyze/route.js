...
1. REQUIRED ELEMENTS — look carefully for each. Mark PRESENT if visible anywhere in the photo, MISSING only if truly absent:
   ...
   - Promo table: a display unit/box with "Get House Help in 10 mins" or "Get House Help" text printed on it, with InstaHelp/UC branding. It is blue or purple. It can look like a cube, box, or barrel. IMPORTANT: if you can read "House Help" or "Get House Help in 10 mins" on any object in the image, that object IS the promo table — mark it as PRESENT. Goodie bags/items are often placed on top of it.
   - Goodie bag: branded bags, pamphlets, packets, cards, or any distribution materials at the stall. IMPORTANT: only flag as MISSING if the promo table and surrounding area are completely bare with absolutely no items/materials of any kind visible. If you can see ANY small items, packets, or materials anywhere near the setup, mark as PRESENT. When in doubt, mark as PRESENT.
   - Gazebo/canopy: a 4-post tent/canopy structure with a fabric roof overhead (usually purple or blue). REQUIRED if outdoors. To determine outdoor vs indoor: open sky/road/vehicles/trees visible = OUTDOOR = gazebo required. Inside building/lobby/hall = INDOOR = gazebo not required. CRITICAL: a PARASOL or UMBRELLA on a single pole is NOT a gazebo. If you only see an umbrella/parasol (even a branded one), the gazebo is still MISSING — flag it. Only a proper 4-post tent/canopy counts as a gazebo.
